...
void printException(std::exception* pex, const char* pszThread)
{
    ...
    if(pex)
        snprintf(pszMessage, sizeof(pszMessage), "EXCEPTION: %s       \n%s       \n%s in %s       \n", typeid(*pex).name(),pex->what(),pszModule, pszThread);
    else
        snprintf(pszMessage, sizeof(pszMessage), "UNKNOWN EXCEPTION       \n%s in %s       \n",pszModule, pszThread);
    printf("\n\n************************\n%s",pszMessage);
    if (wxTheApp)
...
void AddTimeData(unsigned int ip, int64 nTime)
{
    ...
    printf("Added time data, samples %d,ip %08x, offset %+I64d minutes)\n", vTimeOffsets.size(), ip, vTimeOffsets.back(), vTimeOffsets. back()/60);
